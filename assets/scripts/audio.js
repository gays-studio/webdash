class ys {
  ...
  startMusic(StartPosOffset = 0) {
    ...
    // Progressive audio loading: load music if not already loaded
    if (!this._scene.cache.audio.exists(_songKey)) {
      // Try to load the audio dynamically
      const musicFile = window.allLevels?.find(lvl => lvl[0] === _songKey);
      let musicPath = null;
      if (musicFile) {
        musicPath = "assets/music/" + (musicFile[4] ? musicFile[4] : musicFile[1].replaceAll(" ", "")) + ".mp3";
      } else {
        musicPath = "assets/music/" + _songKey + ".mp3";
      }
